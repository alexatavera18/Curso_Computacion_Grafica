...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "P3_Proyecciones y transformaciones basicas_Alexa Lopez", nullptr, nullptr);
	...
	*/
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		view = glm::translate(view, glm::vec3(0.0f, .0f, -20.5f));
		...
		model = glm::rotate(model, glm::radians(160.0f), glm::vec3(0, 1, 0));  // use to compare orthographic and perspective projection
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1,0, 0));  // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::rotate(model, glm::radians(60.0f), glm::vec3(0, 1, 0)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::rotate(model, glm::radians(360.0f), glm::vec3(1, 0, 0));
		model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		/*Acomodar las cajas como piramide cada bloque con una letra de tu nombre Alexa (5) cajas
		Al menos que se vea cada cara de los bloques que existen
		diferentes escalas y diferente posisció*/
